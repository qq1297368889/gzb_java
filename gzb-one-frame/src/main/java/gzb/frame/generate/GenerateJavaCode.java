...
import gzb.frame.db.v2.DataBaseConfig;
import gzb.tools.*;
...
public class GenerateJavaCode {
    ...
    public static void generateCode(String codeSrcPath, String pkg, int type, boolean updateMapping, String frameDbKey, String... userDbKeys) throws Exception {
        ...
        DataBase frameDataBase = new DataBase(dataBaseConfig);
        ...
        for (int i = 0; i < userDbKeys.length; i++) {
            ...
            DataBase userDataBase = new DataBase(dataBaseConfigUser);
    ...
    public static void update_permission(DataBase dataBase, List<TableInfo> listTableInfo) throws Exception {
        ...
        for (TableInfo tableInfo : listTableInfo) {
            if (id == null) {
                listGzbMap = dataBase.queryMap("select sys_permission_id from sys_permission where sys_permission_name=? and sys_permission_sup=? and sys_permission_type = ?", new Object[]{tableInfo.dbNameLowerCase, 0, 1});
            ...
            listGzbMap = dataBase.queryMap("select sys_permission_id from sys_permission where sys_permission_name=? and sys_permission_sup=? and sys_permission_type = ?", new Object[]{tableInfo.nameHumpLowerCase, id, 1});
            if (listGzbMap.size() == 0) {
                sql = "INSERT INTO sys_permission(" +
                        "sys_permission_id, sys_permission_name, sys_permission_data, sys_permission_type, " +
                        "sys_permission_desc, sys_permission_sup, sys_permission_sort) VALUES (" +
    ...
    public static void update_mapping(DataBase dataBase, List<TableInfo> listTableInfo) throws Exception {
        ...
        for (TableInfo tableInfo : listTableInfo) {
            listGzbMap = dataBase.queryMap("select sys_mapping_table_id from sys_mapping_table where " +
            ...
            listGzbMap = dataBase.queryMap("select * from sys_option_request where sys_option_request_key = ?", new Object[]{key});
            ...
            listGzbMap = dataBase.queryMap("select * from sys_option_sql where sys_option_sql_key = ?", new Object[]{key});
            if (listGzbMap.size() == 0) {
            ...
            for (int i = 0; i < tableInfo.columnNames.size(); i++) {
                ...
                //生成映射元数据
                listGzbMap = dataBase.queryMap("select * from sys_mapping_column where " +
                        "sys_mapping_column_name=?", new Object[]{name});
                map0.put(tableInfo.nameHumpLowerCase + "." + name, "0");
                if (listGzbMap.size() == 0) {
                    String option = "";
                    //检查列是否 可能是某种选项 sys_option
                    listGzbMap = dataBase.queryMap("select sys_option_id from sys_option where sys_option_key=?", new Object[]{name});
                    if (listGzbMap.size() > 0) {
                        option = name;
                    }
                    if (listGzbMap.size() == 0) {
                    ...
                    //检查 是不是某种引用
                    String request="";
                    listGzbMap = dataBase.queryMap("select sys_option_request_id from sys_option_request where sys_option_request_key=?", new Object[]{name});
                    if (listGzbMap.size() > 0) {
                        request = name;
                    }
                    String sql_op="";
                    listGzbMap = dataBase.queryMap("select sys_option_sql_id from sys_option_sql where sys_option_sql_key=?", new Object[]{name});
                    if (listGzbMap.size() > 0) {
                        sql_op = name;
                    }
                    ...
                    if (id!=null&& this_id!=null
                        ...
                        dataBase.execute( "update sys_mapping_column set sys_mapping_column_table=? " +
        ...
        //删除失效的列或表
        listGzbMap = dataBase.queryMap("select sys_mapping_table_id,sys_mapping_table_name from sys_mapping_table", new Object[]{});
        for (GzbMap gzbMap : listGzbMap) {
            ...
            if (map0.get(t_name) == null) {
                dataBase.execute("delete from sys_mapping_table where sys_mapping_table_id = ?", new Object[]{t_id});
            }
            List<GzbMap> listGzbMap2 = dataBase.queryMap("select sys_mapping_column_id,sys_mapping_column_name from sys_mapping_column where sys_mapping_column_table = ?", new Object[]{t_id});
            for (GzbMap gzbMap2 : listGzbMap2) {
                ...
                if (map0.get(t_name + "." + t_name2) == null) {
                    dataBase.execute("delete from sys_mapping_column where sys_mapping_column_id = ?", new Object[]{t_id2});
                }
            }
        }
        //检查角色专属映射
        listGzbMap = dataBase.queryMap("select sys_role_table_id,sys_role_table_name from sys_role_table", new Object[]{});
        for (GzbMap gzbMap : listGzbMap) {
            String t_name = gzbMap.getString("sysRoleTableName");
            Long t_id = gzbMap.getLong("sysRoleTableId");
            if (map0.get(t_name) == null) {
                dataBase.execute("delete from sys_role_table where sys_role_table_id = ?", new Object[]{t_id});
            }
            List<GzbMap> listGzbMap2 = dataBase.queryMap("select sys_role_column_id,sys_role_column_name from sys_role_column where sys_role_column_table = ?", new Object[]{t_id});
            for (GzbMap gzbMap2 : listGzbMap2) {
                String t_name2 = gzbMap2.getString("sysRoleColumnName");
                Long t_id2 = gzbMap.getLong("sysRoleColumnId");
                if (map0.get(t_name + "." + t_name2) == null) {
                    dataBase.execute("delete from sys_role_column where sys_role_column_id = ?", new Object[]{t_id2});
                }
            }
        }
