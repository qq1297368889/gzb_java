...
public class TCPTools {
    ...
    private static final Map<Integer, PacketEntity> sessionMap = new ConcurrentHashMap<>();
        ...
        ServiceThread.start(1, "ByteTools-Cleanup", new Runnable() {
            ...
            public void run() {
                while (true) {
                    ...
                    Iterator<Map.Entry<Integer, PacketEntity>> it = sessionMap.entrySet().iterator();
                    while (it.hasNext()) {
                        Map.Entry<Integer, PacketEntity> entry = it.next();
