# 2026-03-02 
#### tcp 和 http统一抽象 实现同一个控制器 支持两种协议访问
#### tcp实现一套私有协议 
#### 第1层协议 由 tcp保证
#### 第2层协议 为 size,data 结构
#### 第3层协议 为 路由地址,请求方式(get,post,put,delete),数据类型(表单,json,文件,body),数据体  
#### 例如 /users/info,0,0,xxx=xxx 会路由到 对应控制器 调用并返回结果 由tcp协议发送回客户端 size,data 协议格式
#### 主动推送消息 由 后台线程(热更新线程) 配合 会话管理 实现 
#### 核心逻辑跑通 细节还未完善.......
#### 更改了一些类的包位置.....
#### 后续 udp ws 逐步支持 

# 2026-02-26 优化若干功能
### 大概就是 **@#%$ .....
### 增加了一些文档 
### 修复一个致命问题 前边由于疏忽 导致对象缓存 可能会出现内存泄漏 和 污染 
### 现在已经修复了 手搓了一个线程安全的对象池 针对字符拼接的场景 
### 嗨 还是疏忽了 没有充分测试 


# 2026-02-26
### 恭喜我自己 完成性能突破 目前性能大幅度提升 N倍提升
### 执行效率 参考日志输出(单位纳秒):"同步" | "总耗时" | "900" | "netty数据获取" | "800" | "框架执行" | "100"
### 完全无水分 计时逻辑见: gzb.frame.factory.v4.FactoryImplV2 debug方法

# 2026-02-24
### 拆分一些功能
### TCP支持 准备开发

# 2026-02-12
* 不支持二进制打包了 因为我发现不能热更新了

# 2026-02-08

## 重构完成 
### 性能优化
* 增加类编译缓存 缓存后 类加载速度 0-3ms
### 下一步支持
* 支持二进制打包，开发中....

### 还有一些功能开发中.....
# 2025-10-02

### 功能新增

* 新增
* 可以指定事件传播深度 防止循环传播

### 性能优化

* 1.优化实体类装在逻辑
* 2.优化web请求参数解析逻辑
* 3.更改动态编译的部分代码效率
* 4.数据库事件的一些链路精简

### 修复已知BUG

* 1.一些BUG小修复
* 2.数据库事件的BUG修复

# 2025-09-30

### 性能优化

* 1.性能提升，0微秒比例上升至60-80%

```java

@DecoratorOpen
@Limitation(10)
@Header(item = {@HeaderItem(key = "content-type", val = "application/json;charset=UTF-8")})
@GetMapping("/test1")
public Object test1(String sysUsersAcc, GzbJson gzbJson) {
    return gzbJson.success(sysUsersAcc);
}
```

### 修复已知BUG

* 1.一些BUG修复

### 功能新增

* 1.支持注册 数据库事件 类似触发器 但是更加灵活 可以给每个实体类绑定 select sava update delete 事件

# 2025-09-27

### 性能优化

* 1.一些性能优化

## 一个基础请求耗时小于1微秒 比例达到 99.99% (框架耗时30% 业务代码70% ，底层耗时没有计算)

```java

@DecoratorOpen
@Limitation(10)
@Header(item = {@HeaderItem(key = "content-type", val = "application/json;charset=UTF-8")})
@GetMapping("/test1")
public Object test1(String sysUsersAcc, GzbJson gzbJson) {
    return gzbJson.success(sysUsersAcc);
}
```

### 修复已知BUG

* 1.一些BUG修复

# 2025-09-26

### 修复已知BUG

* 1.上传文件未及时删除，可能泄露
* 2.修复 FileUploadEntity 参数接收不到文件的情况
* 3.多装饰器的调用顺序通过 sort指定 防止调用顺序和预期不符 @DecoratorStart(sort = 0) @DecoratorEnd(sort = 0) sort越小越先执行
* 4.修复多装饰器时，部分装饰器失效的问题
* 5.修复 gzb.system.server.http.post.size 配置项不生效

### 功能改进

* 1.当检测到 数据库异步线程被设置为0的时候，弹出警告，防止出现难以排查问题
* 2.配置文件中配置的所有 classpath:/xx,this:/xx 目录，不存在则 自动创建，防止出现意外错误
* 3.增加更详细的错误输出
