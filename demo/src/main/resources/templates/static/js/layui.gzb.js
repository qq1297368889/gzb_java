...
gzb.jsonVerify = function (json, successMsg, sucFun, failFun, errFun, jumpFun) {
    ...
    if (status.toString() === gzb.status["success"]) {
        if (successMsg) {
            gzb.msgSuccess(msg);
        }
        if (sucFun != null) {
    ...
    if (status.toString() === gzb.status["fail"]) {
        gzb.msgErr(msg);
        if (failFun != null) {
            failFun();
        }
    ...
    if (status.toString() === gzb.status["err"]) {
        gzb.msgErr(msg);
        if (errFun != null) {
            errFun();
        }
    ...
    if (status.toString() === gzb.status["jump"]) {
...
gzb.initTableBut = function (res, table_cols) {
    ...
    if (but_table_html.length > 0) {
        ...
        console.log("json1.width",json1.width)
