...
import com.frame.entity.SysGroupPermission;
...
@RequestMapping(value = "/system/v1.0.0/sysGroupPermission")
public class SysGroupPermissionController{
    @Resource(value = "com.frame.dao.impl.SysGroupPermissionDaoImpl")
    SysGroupPermissionDao sysGroupPermissionDao;
    @Resource(value = "gzb.tools.log.LogImpl")
    Log log;

    /**
     * 查询,不能接受空数据,请至少提供一个参数,请提供 SysGroupPermission的参数 
     * /system/v1.0.0/sysGroupPermission/find?参数同下,只是仅接受查询结果为1条的情况
     * */
    @DecoratorOpen
    @GetMapping(value = "find")
    public Object find(GzbJson result, SysGroupPermission sysGroupPermission) throws Exception {
        ...
        return result.success("查询成功",sysGroupPermission1);
    }
    ...
    @DecoratorOpen
    @GetMapping("list")
    public Object list(GzbJson result,SysGroupPermission sysGroupPermission, String sortField, String sortType, Integer page, Integer size) throws Exception {
        if(sysGroupPermission == null){
           sysGroupPermission = new SysGroupPermission();
        }
        return sysGroupPermissionDao.queryPage(sysGroupPermission, sortField, sortType, page, size, 100, 1000);
    }

    /**
     * 查询,根据id修改其他参数 请提供 SysGroupPermission的参数 
    ...
    public Object query(GzbJson result, String[] field, String[] symbol, String[] value, String[] montage, String sortField, String sortType, Integer page, Integer limit) throws Exception {
        SqlTemplate sqlTemplate = sysGroupPermissionDao.getDataBase().toSelect("sys_group_permission", field, symbol, value, montage, null, null);
    ...
    @DecoratorOpen
    @PostMapping("save")
    public Object save(GzbJson result, SysGroupPermission sysGroupPermission) throws Exception {
        if (sysGroupPermission == null || sysGroupPermission.toString().equals("{}")) {
    ...
    @DecoratorOpen
    @PostMapping("update")
    public Object update(GzbJson result, SysGroupPermission sysGroupPermission) throws Exception {
    ...
    public Object delete(GzbJson result, SysGroupPermission sysGroupPermission) throws Exception {
        if (sysGroupPermission == null || sysGroupPermission.toString().equals("{}") || sysGroupPermission.getSysGroupPermissionId() == null) {
    ...
    public Object deleteAll(GzbJson result, java.lang.Long[] sysGroupPermissionId) throws Exception {
        if (sysGroupPermissionId == null || sysGroupPermissionId.length < 1) {
            return result.fail("delete 输入参数错误");
        }
        int num = 0;
        SysGroupPermission frame = new SysGroupPermission();
        for (java.lang.Long _id : sysGroupPermissionId) {
            sysGroupPermissionDao.delete(frame.setSysGroupPermissionId(_id));
            num++;
        }
