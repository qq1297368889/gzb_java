...
public class SysGroupController{
    ...
    public Object query(GzbJson result, String[] field, String[] symbol, String[] value, String[] montage, String sortField, String sortType, Integer page, Integer limit) throws Exception {
        SqlTemplate sqlTemplate = sysGroupDao.getDataBase().toSelect("sys_group", field, symbol, value, montage, null, null);
    ...
    public Object deleteAll(GzbJson result, java.lang.Long[] sysGroupId) throws Exception {
        if (sysGroupId == null || sysGroupId.length < 1) {
            return result.fail("delete 输入参数错误");
        }
        int num = 0;
        SysGroup frame = new SysGroup();
        for (java.lang.Long _id : sysGroupId) {
            sysGroupDao.delete(frame.setSysGroupId(_id));
            num++;
        }
