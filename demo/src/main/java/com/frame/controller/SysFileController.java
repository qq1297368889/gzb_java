...
public class SysFileController{
    ...
    public Object query(GzbJson result, String[] field, String[] symbol, String[] value, String[] montage, String sortField, String sortType, Integer page, Integer limit) throws Exception {
        SqlTemplate sqlTemplate = sysFileDao.getDataBase().toSelect("sys_file", field, symbol, value, montage, null, null);
    ...
    public Object deleteAll(GzbJson result, java.lang.Long[] sysFileId) throws Exception {
        if (sysFileId == null || sysFileId.length < 1) {
            return result.fail("delete 输入参数错误");
        }
        int num = 0;
        SysFile frame = new SysFile();
        for (java.lang.Long _id : sysFileId) {
            sysFileDao.delete(frame.setSysFileId(_id));
            num++;
        }
