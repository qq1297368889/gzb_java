...
* 序列化性能（额外领先 34%）：从单对象获取场景看，gzb-one 处理 POJO 转 JSON 的内存拷贝损耗远低于 Quarkus。
* 参数解析性能（额外领先 59%）：在多参数映射场景，gzb-one 消除了参数绑定的开销。
* 数组解析 ：这个情况比较复杂 它涉及到优势累加 还涉及到大相应的额外开销，不好算
...
#### 强调一下 P99在框架中意义不大，并不意味着业务中也没意义 我没有这么说。业务场景中可能会有一些特殊的瓶颈点，这时候P99就很有意义了。
#### Quarkus 的延迟高 应该是运气不好遇到系统调度影响了 我以前测试过很多遍 这个数据我都会背了 正常5ms左右才对 当5ms看就行
#### 但是这是自动脚本测试出来的，所以我也不好随便改数据了，毕竟我也不想被喷说数据造假（虽然我确实想改掉这个数据）。
...
|  | /test/users/1       | 19,796 | 87.7% | 846.3 | 5.21ms | 我.爱你 |
...
- **Quarkus** P99 小于 **10.1ms 以内（应该是机器调度波动，5ms左右才对，因为我测过很多遍，这些数据我都会背了）**。
...
- **前端编译** 有些处理和判断 gzb.one 选择在编译期 处理判断 核心链路 是一条直线大马路
- **拒绝反射** 核心链路无反射 全靠前端编译器续命 启动时和检测到需要 **无缝热更新** 时，框架会使用前端编译器处理你的类
