...
public class SysLog implements Serializable, JsonSerializable{
    ...
    @EntityAttribute(key=true,size = 0,name="sys_log_id",desc="日志ID",type="bigint")
    private Long sysLogId;
    @EntityAttribute(key=false,size = 0,name="sys_log_time",desc="时间",type="datetime")
    private java.time.LocalDateTime sysLogTime;
    @EntityAttribute(key=false,size = 0,name="sys_log_ms",desc="耗时",type="bigint")
    private Long sysLogMs;
    @EntityAttribute(key=false,size = 0,name="sys_log_sql",desc="sql",type="longtext")
    private String sysLogSql;
    private Object data;
   public SysLog() {}
    ...
    public SysLog setSysLogId(Long sysLogId) {
    ...
    public Long getSysLogMs() {
        return sysLogMs;
    }
    public SysLog setSysLogMs(Long sysLogMs) {
        this.sysLogMs = sysLogMs;
        return this;
    }
    public String getSysLogSql() {
        return sysLogSql;
    }
    public SysLog setSysLogSql(String sysLogSql) {
        int size0 = Tools.textLength(sysLogSql);
        if (size0 > 0) {
            throw new gzb.exception.GzbException0("SysLog.sysLogSql最大长度为:0,实际长度为:"+ size0 +",数据为:"+sysLogSql);
        }
        this.sysLogSql = sysLogSql;
        return this;
    }
    public SysLog setSysLogSqlUnsafe(String sysLogSql) {
        this.sysLogSql = sysLogSql;
        return this;
    }
