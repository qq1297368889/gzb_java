...
public class SysRoleEvent {
    ...
    Log log;
    @Resource
    SysRoleDao sysRoleDao;//不仅限于这个对应的dao 其他任何 service都可以被注入

    //entity 注册到 某个实体类 数据库 新增事件
    //executionBefore 事件执行时机，true 主操作执行前 false 主操作执行后 默认false
    //depth 事件传播深度 防止循环传播
    @DataBaseEventUpdate(entity = SysRole.class, executionBefore = false, depth = 5)
    public void update(
            SysRole sysRole, //对应的实体类对象
            Request request //来自上下文中的对象 和 controller 注入规则一致
    ) throws Exception {

    }

    @DataBaseEventSelect(entity = SysRole.class, executionBefore = false, depth = 5)
    public void select(SysRole sysRole) throws Exception {

    }

    @DataBaseEventSave(entity = SysRole.class, executionBefore = false, depth = 5)
