...
import com.frame.service.BaseDecorator;
...
@Decorator
public class HookAction extends BaseDecorator {

    @DecoratorEnd("/system/")
    public RunRes testEnd(RunRes runRes) {
        runRes.setState(200); //允许通行 默认允许
        //runRes.setState(400); //禁止通行
        //runRes.getData();//获取原响应数据
        //runRes.setData("new data");//修改响应数据
        return runRes;
    }
    //验证例子
